...
def load_model_sharded(model, rank, cfg):
    ...
    model_name_local = cfg.model_name.split('/')[-1]
    folder_name = (
...
def save_model_checkpoint(
    ...
    if rank == 0:
        ...
        os.mkdirs(save_dir)
        save_name = model_name_local + "-" + str(epoch) + ".pt"
...
def save_model_checkpoint_nofsdp(
    model,
    cfg,
    epoch=1,
):
...
def load_model_checkpoint(model, rank, cfg):
    ...
    full_state_dict_model_path = (
        cfg.checkpoint_folder / cfg.checkpoint_model_filename # Path.cwd() / 
    )
    # is it present...
...
def save_optimizer_checkpoint_nofsdp(optimizer, cfg, epoch=1):
    ...
    opt_save_name = (
        "optimizer" + "-" + model_name_local + "-" + str(epoch) + ".pt"
    )
